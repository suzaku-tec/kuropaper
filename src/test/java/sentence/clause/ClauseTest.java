...
public class ClauseTest {
    ...
    public void testPredicate4() throws Exception {
        ...
        assertEquals("述語判定エラー", Arrays.asList("登りながら、", "考えた。"), c.getPredicate().stream().map(Paragraph::getParagraph).collect(Collectors.toList()));
    ...
    public void testSimilarity1() throws Exception {
        ...
        paragraph = c.getParagraphList().get(3);// 「作品の」の文節を取得
    ...
    public void testPartsSubject() throws ParagraphException {
        ...
        assertEquals("主語数エラー：" + getParagraphListStr(c), 1, c.getSubjects().size());
        assertEquals("述語数エラー：" + getParagraphListStr(c) + ": 述語文節 = " + c.getPredicate().stream().map(Paragraph::getParagraph).collect(Collectors.joining("/")), 1, c.getPredicate().size());
    ...
    public void testModifiedWord3() throws ParagraphException {
        ...
        assertEquals("修飾語数エラー:" + result.stream().map(Paragraph::getParagraph).collect(Collectors.joining("-")), 1, result.size());
        assertEquals("多くの", result.get(0).getParagraph());
    }
