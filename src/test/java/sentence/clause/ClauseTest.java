...
public class ClauseTest {
    ...
    public void testParagraph1() throws ParagraphException {
        ...
        assertEquals("文節区切り位置不正 : " + paragraphListStr
                ...
                , c.getPhraseList().stream().map(paragraph -> paragraph.getPharseStr()).collect(Collectors.toList()));
    }
    ...
    public void testSimilarity1() throws Exception {
        ...
        assertEquals("修飾語判定エラー", "もっとも", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));

        phrase = c.getPhraseList().get(3);// 「作品の」の文節を取得
        assertEquals("修飾語判定エラー", "優れた", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));

        phrase = c.getPhraseList().get(4);// 「一つだ。」の文節を取得
        assertEquals("修飾語判定エラー", "作品の", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));
    ...
    public void testMultiSubject() throws ParagraphException {
        ...
        assertEquals("修飾語判定エラー", "家の", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));

        phrase = c.getPhraseList().get(3); // 「手伝い、」の文節取得
        assertEquals("修飾語判定エラー", "掃除を", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));

        phrase = c.getPhraseList().get(6); // 「ゲームを」の文節取得
        assertEquals("修飾語判定エラー", "部屋で", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));

        phrase = c.getPhraseList().get(7); // 「する。」の文節取得
        assertEquals("修飾語判定エラー", "ゲームを", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));
    }
    ...
    public void testPartsSubject() throws ParagraphException {
        ...
        assertEquals("修飾語判定エラー", "生まれた", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));

        phrase = c.getPhraseList().get(4); // 「名産地です。」の文節取得
        assertEquals("修飾語判定エラー", "リンゴの", phrase.getSimilarities().stream().map(p -> p.getPharseStr()).collect(Collectors.joining()));
    }
